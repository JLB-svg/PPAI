...
**Version:** 1.4 (Documentation & Filename Clarification Edition)  
**Date:** May 13, 2026  
...
**Purpose:** Provide a single, unified style authority for **every** file type in the PPAI-Brain repository, **including the entire /docs/ folder and all Markdown documentation files**. High-level principles apply universally; short subsections give precise rules per file type. This v1.4 makes documentation scope explicit and strengthens filename rules for all docs.
...
- **Documentation Files (/docs/ folder)**: Must follow the same rules. Core system documents should use the pattern `PPAI_[DESCRIPTIVE_NAME]_vX.Y.md`. Supporting documents use `snake_case` or consistent `Topic_vX.Y.md`. Existing files with spaces (e.g. `Brain Environment Architecture.md`) must be renamed during the next cleanup pass.
...
- **References to Specs**: Always use the exact filename + version (e.g. `per PPAI_Code_Style_Guide.md v1.4 – Documentation & Filename Clarification`).
...
- Updates to this document follow the Collaboration Protocol (targeted diff only, example below):

```diff
## 1. Universal Principles
- Token Efficiency: All human- or AI-readable content must be concise...
+ Token Efficiency as First-Order Priority: All human- or AI-readable content must be concise, predictable, and low-noise. Eliminate redundancy aggressively.
