...
**References:** `PPAI_Collaboration_Protocol.md` v1.0, `PPAI_SYSTEM_ARCHITECTURE_v2.2.md` – Brian’s Philosophy, `BRAIN_ENVIRONMENT_ARCHITECTURE_v2.1.md`  
...
- **Traceability First**: Every file, function, section, or cell must contain a one-line reference to the exact spec (e.g., `# per PPAI_SYSTEM_ARCHITECTURE_v2.2.md – Brian’s Philosophy` or `<!-- per … -->`).
...
Add this file to `PPAI-Brain/docs/`. All future code, docs, configs, data files, and artifacts will follow it by default (including the new Naming Conventions rules).

This update continues our pattern of high-ROI, token-saving improvements while preserving Brian’s Philosophy and xAI best practices.
